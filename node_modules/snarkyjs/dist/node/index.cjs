...
var Poseidon = {
  ...
  update(state3, input) {
    ...
    let newState = Snarky.poseidon.update(MlFieldArray.to(state3), MlFieldArray.to(input), isChecked);
...
var Hash = createHashHelpers(Field4, Poseidon);
...
var Hash2 = createHashHelpers(Field, Poseidon2);
...
var { Events, Actions } = createEvents({ Field: Field4, Poseidon });
var ActionState = {
  ...provable2(Field4),
...
var { Events: Events2, Actions: Actions2 } = createEvents({ Field, Poseidon: Poseidon2 });
var ActionState2 = {
  ...Field,
...
var MerkleTree = class {
  ...
  constructor(height) {
    this.height = height;
    this.nodes = {};
    this.zeroes = [Field4(0)];
    for (let i2 = 1; i2 < height; i2++) {
      this.zeroes.push(Poseidon.hash([this.zeroes[i2 - 1], this.zeroes[i2 - 1]]));
    }
  }
  /**
  ...
  validate(index) {
    ...
    for (const node of path) {
      hash2 = Poseidon.hash(node.isLeft ? [hash2, node.sibling] : [node.sibling, hash2]);
    }
...
var BaseMerkleWitness = class extends CircuitValue {
  ...
  calculateRoot(leaf) {
    ...
    for (let i2 = 1; i2 < n; ++i2) {
      ...
      hash2 = Poseidon.hash([left, right]);
    }
    return hash2;
  }
  /**
...
var Nullifier = class extends Struct({
   ...
   */
  key() {
    return Poseidon.hash(Group3.toFields(this.public.nullifier));
  }
  /**
